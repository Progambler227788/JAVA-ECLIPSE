	...
	static int addWordToSortedList(String inWord) {
		...
		// Be sure to count comparisons and moves
		 ...
		 int insertIndex = -1;
		    ...
		    // Linear search for insertion point using selection sort
		    for (int i = 0; i < lastIndex; i++) {

		    	compareStringCount++; // Increment comparison count
		        if (words[i].equals(inWord)) {
		            count[i]++; // Increment count if word found
		            foundWord = true;
		            break;
		        } else if (words[i].compareTo(inWord) > 0) {
		            insertIndex = i; // Update index to maintain sorted order
		            break;
		    ...
		    // If insertIndex remains -1, insert at the end
		    if (insertIndex == -1) {
